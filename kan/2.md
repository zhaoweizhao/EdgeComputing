...
```python
avg=19.29570827012062

Wrote profile results to tttttt.py.lprof
Timer unit: 1e-06 s

Total time: 189.768 s
...
==============================================================
    ...
    96     10100      77684.0      7.7      0.0          B = x.shape[0]
    97     10100    5451331.4    539.7      2.9          x = self.patch_embed(x)
    98     10100     460184.8     45.6      0.2          cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
    99     10100     306842.2     30.4      0.2          dist_token = self.dist_token.expand(B, -1, -1)
   100     10100    1078875.1    106.8      0.6          x = torch.cat((cls_tokens, dist_token, x), dim=1)
   101     10100     829477.5     82.1      0.4          x = x + self.pos_embed
   102     10100     660335.7     65.4      0.3          x = self.pos_drop(x)
   103     10100      16182.2      1.6      0.0          intermediate = [0]
   104
   105     67189     368708.7      5.5      0.2          for blk_idx, blk in enumerate(self.blocks):
   106     67032  112763220.1   1682.2     59.4              x = blk.forward(x)
   107     67032     279604.3      4.2      0.1              if hasattr(self, 'intermediate_head_positions') and blk_idx in self.intermediate_head_positions:
   108     66875    7817236.2    116.9      4.1                  inter_z = self.norm(x)
   109     66875   33375340.5    499.1     17.6                  inter_logit = (self.intermediate_heads[blk_idx](inter_z[:, 0]) + self.intermediate_heads_dist[blk_idx](inter_z[:, 1])) / 2
   110     66875    6467022.9     96.7      3.4                  p = inter_logit + intermediate[-1] * self.ensemble_reweight[blk_idx]
   111     66875     185636.0      2.8      0.1                  intermediate.append(p)
   112     66875    5408355.5     80.9      2.8                  pred = torch.nn.functional.softmax(p)
   113     66875    5768675.4     86.3      3.0                  max_pred, argmax_pred = torch.max(pred, dim=1)
   114                                                           # print("******",max_pred.shape)
   115                                                           # print(max_pred)
   116     66875    8317094.9    124.4      4.4                  if max_pred >= self.threshold:
   117      9943      28553.6      2.9      0.0                      return inter_logit
   118       157      18062.3    115.0      0.0          x = self.norm(x)
   119       157      73059.9    465.3      0.0          x = (self.head(x[:, 0]) + self.head_dist(x[:, 1])) / 2
   120       157      16107.1    102.6      0.0          p = x + intermediate[-1] * self.ensemble_reweight[-1]
   121       157        145.1      0.9      0.0          return x
```
