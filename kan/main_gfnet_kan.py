...
import json
import sys
sys.path.append("/home/heshaowei/GFNet/")
...
from engine import train_one_epoch, evaluate, train_one_epoch_boosted, test_boosted
from losses import DistillationLoss
from samplers import RASampler
import utils
# from gfnet import GFNet, GFNetPyramid
# from gfnet_boosted import GFNet, GFNetPyramid
from models_kan import GFNet_KAN_Boosted
...
def freeze_backbone(network, excluded_submodules: list[str]):
    ...
    total_num_parameters = sum([np.prod(p.size()) for p in model_parameters]) #计算给定数组中所有元素的乘积
    ...
    for submodule_attr_name in excluded_submodules:  # Unfreeze excluded submodules to be trained.
        for submodule in getattr(network, submodule_attr_name):
...
def get_args_parser():
    ...
    # * Finetuning params
    parser.add_argument('--finetune', default='/home/heshaowei/GFNet/logs/gfnet-xs/Epoch16_acc80.43.pth', help='finetune from checkpoint')
    ...
    parser.add_argument('--resume', default='/home/heshaowei/GFNet/logs/gfnet_b_kan/Epoch20_acc43.55.pth', help='resume from checkpoint')
...
#**************************************************************************************
    model = GFNet_KAN_Boosted(
        img_size=args.input_size, 
        patch_size=16, embed_dim=512, depth=19, drop_path_rate=0.25,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), hdim_kan=192
    )
#**************************************************************************************

    if args.finetune and False:
    ...
    print(f"Starting from the {args.start_epoch} epoch")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # )
        train_stats = train_one_epoch_boosted(
        ...
        test_stats = test_boosted(model, data_loader_val, epoch, device=device)
        acc = test_stats[-1]
        ...
        if True:
            checkpoint_path = output_dir / f'Epoch{epoch}_acc{acc:.2f}.pth'
            if model_ema is not None:
                utils.save_on_master({
                    'model': model_without_ddp.state_dict(),
                    'optimizer': optimizer.state_dict(),
